...
dim(counts)
dim(counts_txt)
counts_txt <- counts_txt  %>% left_join(ids%>% dplyr::select(-ENTREZID)) 
counts_txt <- counts[!duplicated(counts$gene),]                 
head()
write.table(counts_txt,"data/ipmn_moonshot_tpm.txt", row.names = FALSE, sep ="\t", quote=FALSE)

counts_txt <- df_tpm %>% rownames_to_column("gene")
counts_txt <- counts_txt[!duplicated(counts_txt$gene),] 
write.table(counts_txt ,
            "data/ipmn_moonshot_count.txt", row.names = FALSE, sep ="\t", quote=FALSE)
...
BE02_case_list <-read_excel("data/cancer_moonshot_case_list_final.xlsx")
str(BE02_case_list)
sort(unique(BE02_case_list$Sample_ID))
...
BE02_case_list$Sample_ID <- paste0("MS",BE02_case_list$Sample_ID)
str(BE02_case_list)

BE02_case_list <- BE02_case_list %>% filter(Sample_ID %in% cols)
df <- BE02_case_list %>%
  group_by(Grade) %>% 
  summarise(n = length(unique(Path_ID)) ) 
...
data.frame(cols=cols, Sample_ID = cols) %>% left_join(BE02_case_list) %>% 
  group_by(Grade) %>% 
  summarise(n = length(unique(Path_ID))) 
...
#df_count <-read.csv("data/tpm_untrimmed.csv", header=TRUE, row.names = 1)
df_count <-read.csv("data/pancreas_counts_min10counts_with_IDs.csv", header=TRUE, row.names = 1)
colnames(df_count) <- stringr::str_replace(colnames(df_count), "X", "MS")
colnames(df_count) <- stringr::str_replace(colnames(df_count), "\\.", "_")
str(df_count)

cols <- colnames(df_count)

df_tpm <- count2tpm(df_count, idType = "symbol")
...
counts <- df_tpm  %>% dplyr::select(condition$Sample_ID)
...
set.seed(1200)
condition$Disease <- factor(condition$Disease, 
...
autoplot(cnt_pca , data = condition,color = "Disease", size = 2) 
  theme_minimal()
plotly::ggplotly(autoplot(cnt_pca, data = condit2F56B5ion,color = "Disease", size = 2.5 ) +
...
# pathways.hallmark <- gmtPathways("~/Documents/Rutgers_CW/De_lab/download_resources/c2.cp.reactome.v2024.1.Hs.entrez.gmt") # for reactome pathways
...
ggplot(fgseaResTidy %>% 
  ...
  mutate(pathway = str_remove(pathway, "REACTOME_")), aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
...
ids_IC<-bitr(immune_genes, fromType="SYMBOL",
             toType="ENSEMBL",
             OrgDb="org.Hs.eg.db") #NA values drop by default
# ids_IC <- ids_IC[-2,]
...
pheatmap::pheatmap(heatmap_df  , cluster_cols = FALSE)
...
st_line <- data.frame(xmin = c(1,4),
                      xmax = c(3,6),
                      y.position = c(p_values$y.position-30, p_values$y.position-30 ),
                      p="",
                      group1=1,
                      group2 =2)

color <- c("#48217390","#433E8590", "#38598C90","#2BB07F90","#85D54A90", "#FDE725FF") ##2BB07F50")
p <- ggboxplot(df_plot , x = "Disease", y = "FA2H",
               # color = "Disease",
